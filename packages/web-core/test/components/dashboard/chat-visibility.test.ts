...
describe('chat visibility', () => {
    it('keeps a larger default live window for cli-like conversations', () => {
        expect(getDefaultVisibleLiveMessages({ isCliLike: false })).toBe(60)
        expect(getDefaultVisibleLiveMessages({ isCliLike: true })).toBe(1000)
    })
    ...
    it('hydrates a matching recent tail window for cli-like conversations on reload/remount', () => {
        ...
        expect(getDefaultChatTailHydrateLimit({ isCliLike: true })).toBe(1000)
    })
})
