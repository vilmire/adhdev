...
describe('CLI terminal measured layout plumbing', () => {
  ...
  it('keeps CLI chat and terminal panes mounted in the same measured slot instead of display-none toggling the terminal', () => {
    ...
    expect(paneGroupSource.includes('const showTerminalPane = isCliTerminal')).toBe(true)
    expect(paneGroupSource.includes('const showChatPane = !isCliTerminal')).toBe(true)
    expect(paneGroupSource.includes('requestAnimationFrame')).toBe(false)
    expect(paneGroupSource.includes('terminalRevealReady')).toBe(false)
    expect(paneGroupSource.includes('const terminalPaneVisible = getPaneGroupContentChildVisibility(isVisible, showTerminalPane)')).toBe(true)
  ...
  it('always requests a fresh runtime snapshot after replaying a hidden buffered snapshot on visibility restore, and replays large terminal buffers in frame-sized chunks instead of one giant write', () => {
    ...
    expect(source.includes('terminal.write(nextChunk, () => {')).toBe(true)
    ...
    expect(source.includes('finishScrollbackReplayIfNeeded();')).toBe(true)
  })
  ...
  it('exposes a manual older-terminal-output loader that force replays raw session scrollback without changing dashboard resize policy', () => {
    ...
    expect(paneSource.includes('const previousIsVisibleRef = useRef(isVisible);')).toBe(true)
    expect(paneSource.includes('const wasVisible = previousIsVisibleRef.current;')).toBe(true)
    expect(paneSource.includes('if (!wasVisible) {\n            setHasLoadedOlderRuntimeScrollback(false);\n        }')).toBe(true)
  ...
  it('exposes terminal viewport scroll control through the renderer and lazy wrapper for older scrollback replay anchoring', () => {
    ...
    expect(terminalSource.includes('write: (data: string, onProcessed?: () => void) => void;')).toBe(true)
    expect(terminalSource.includes('terminalRef.current.write(data, onProcessed)')).toBe(true)
    expect(terminalSource.includes('term.write(chunk.data, chunk.onProcessed)')).toBe(true)
    ...
    expect(wrapperSource.includes('write: (data: string, onProcessed?: () => void) => {')).toBe(true)
    expect(wrapperSource.includes('innerRef.current.write(data, onProcessed)')).toBe(true)
    expect(wrapperSource.includes('terminal.write(chunk.data, chunk.onProcessed)')).toBe(true)
    expect(wrapperSource.includes('pendingWritesRef.current.push({ data, onProcessed })')).toBe(true)
