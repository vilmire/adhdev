...
describe('dashboard mesh graph dialog wiring', () => {
  ...
  it('drives the dialog from live mesh_status data and transforms it through buildMeshGraph', () => {
    ...
    expect(dialogSource).toContain('hasPendingDashboardMeshRefresh(meshStatus.nodes)')
    expect(dialogSource).toContain('nextDashboardMeshRefreshDelayMs(pendingRefreshAttemptRef.current)')
    expect(dialogSource).toContain('pendingRefreshTimerRef.current = window.setTimeout(() => {')
    expect(dialogSource).toContain('void loadGraph()')
