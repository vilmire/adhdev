...
describe('dashboard mesh graph dialog wiring', () => {
  ...
  it('keeps node drill-down on live status rows instead of reusing the orphan graph panel copy', () => {
    ...
    expect(surfaceSource).toContain("t('meshGraph.obs.keyWarning')")
    expect(surfaceSource).toContain("t('meshGraph.obs.followUpLabel")
  ...
  it('keeps node drill-down panels on the shared mesh-theme helper', () => {
    ...
    expect(panelSource).toContain("Field label={t('meshGraph.panel.fieldHead')} value={headSummary} rowClass={meshTheme.panelFieldRowClass}")
  ...
  it('adds direct session/chat affordances and on-demand git history to the observability detail flow', () => {
    ...
    expect(surfaceSource).toContain("Row label={t('meshGraph.obs.fieldHead')}")
    expect(surfaceSource).toContain("Row label={t('meshGraph.obs.fieldSessions')}")
  ...
  it('keeps mobile and desktop entry points on the shared graph popup surface', () => {
    ...
    expect(mainViewSource).toContain('activeConv={meshGraphConversation}')
    expect(surfaceSource).toContain("t('meshGraph.obs.selectedNode')")
    expect(surfaceSource).toContain('resolveSelectedGraphNodeForDetail(canonicalGraph, selectedNodeId)')
    expect(surfaceSource).toContain("Row label={t('meshGraph.obs.fieldDirtyAheadBehind')}")
    expect(surfaceSource).toContain("Row label={t('meshGraph.obs.fieldSource')}")
    expect(surfaceSource).toContain("Row label={t('meshGraph.obs.fieldTransport')}")
    expect(surfaceSource).not.toContain('<MeshGraphPanel')
  })
})
