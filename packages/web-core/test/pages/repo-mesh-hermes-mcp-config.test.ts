...
describe('RepoMesh graph detail affordances', () => {
  ...
  it('preserves provider priority when provider inventory is unavailable and excludes worktree nodes from the settings list', () => {
    // Per-node provider priority editing was removed as a dead path (slots are the
    // editing surface now — NodeSlotEditor). The add-node handler in
    // useMeshNodeActions.ts still persists the FULL chosen order (dedup only),
    // never filtered to the inventory detected on this machine — the guarantee
    // this test exists to protect.
    const nodeActionsSource = readSource('pages/repo-mesh/useMeshNodeActions.ts')
    expect(nodeActionsSource).toContain('payload.providerPriority = normalizeProviderPriority(nodeProviderPriority)')
