...
/** Stable difficulty keys (the data axis). Labels/hints are resolved from i18n. */
const DIFFICULTY_KEYS = ['easy', 'medium', 'difficult', 'freeform'] as const
...
export default function DifficultyBrainEditor({ daemonId, sendDaemonCommand, providerOptions }: Props) {
    ...
    const difficulties = useMemo(
        () => DIFFICULTY_KEYS.map(key => ({
            key,
            label: t(`meshGraph.difficulty.${key}`),
            hint: t(`meshGraph.difficulty.${key}Hint`),
        })),
        [t],
    )
        ...
        <div className="flex flex-col gap-3 p-1">
                        ...
                        <div key={key} className={`${meshTheme.cardClass} rounded-2xl p-4 flex flex-col gap-2`}>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                                <label className="flex flex-col gap-1">
                                    <span className={labelClass}>{t('meshGraph.difficulty.provider')}</span>
                                    <select className={inputClass} value={s.provider} onChange={e => update(key, { provider: e.target.value })}>
                                        <option value="">{t('meshGraph.difficulty.routedDefault')}</option>
                                        {providers.map(p => <option key={p} value={p}>{p}</option>)}
                                        {s.provider && !providers.includes(s.provider) && <option value={s.provider}>{s.provider}</option>}
                                    </select>
                                </label>
                                <label className="flex flex-col gap-1">
                                    <span className={labelClass}>{t('meshGraph.difficulty.model')}</span>
                                    <input className={inputClass} value={s.model} placeholder={t('meshGraph.difficulty.modelPlaceholder')}
                                        onChange={e => update(key, { model: e.target.value })} />
                                </label>
                                <label className="flex flex-col gap-1">
                                    <span className={labelClass}>{t('meshGraph.difficulty.thinking')}</span>
                                    <select className={inputClass} value={s.thinkingLevel} onChange={e => update(key, { thinkingLevel: e.target.value })}>
                                        {THINKING_LEVELS.map(l => <option key={l || 'default'} value={l}>{l || t('meshGraph.difficulty.defaultOption')}</option>)}
                                    </select>
                                </label>
                            </div>
                        </div>
            ...
            <div className="flex items-center gap-2">
                <button type="button" className="btn btn-primary btn-sm" onClick={() => void save()} disabled={saving || loading || !canCommand || !dirty}>
                    {saving ? t('meshGraph.difficulty.saving') : t('meshGraph.difficulty.savePresets')}
                </button>
                <button type="button" className="btn btn-secondary btn-sm" onClick={() => void load()} disabled={loading || saving || !canCommand}>
                    {loading ? t('meshGraph.difficulty.loading') : t('meshGraph.difficulty.reset')}
                </button>
            </div>
        </div>
    )
