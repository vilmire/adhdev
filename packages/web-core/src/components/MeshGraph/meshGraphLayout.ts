...
export function formatMeshGraphAheadBehindLocalized(node: MeshGraphNode, t?: MeshSummaryTranslator): string | null {
    ...
    if (node.ahead > 0 && node.behind > 0) {
        return trOr(t, 'mesh.attention.aheadBehind', `ahead ${node.ahead} / behind ${node.behind}`, { ahead: node.ahead, behind: node.behind })
    }
    if (node.behind > 0) return trOr(t, 'mesh.attention.behind', `behind ${node.behind}`, { count: node.behind })
    return trOr(t, 'mesh.attention.ahead', `ahead ${node.ahead}`, { count: node.ahead })
}
...
export function getNodeSummaryForLayout(node: MeshGraphNode, t?: MeshSummaryTranslator): string {
    ...
    if (node.type === 'submoduleNode') {
        const parts = [node.machineLabel ? trOr(t, 'mesh.summary.parent', `parent ${node.machineLabel}`, { label: node.machineLabel }) : null]
        if (node.outOfSync) parts.push(trOr(t, 'mesh.panel.outOfSyncBadge', 'out of sync'))
        else if (node.dirty) parts.push(trOr(t, 'mesh.summary.localChanges', 'local changes'))
        else parts.push(trOr(t, 'mesh.panel.submoduleSynced', 'synced'))
    ...
    if (node.upstream && node.upstreamStatus !== 'fresh') {
        parts.push(trOr(t, 'mesh.attention.upstreamUnverified', 'upstream unverified'))
    } else {
    ...
    if (parts.length === 0) {
        return node.branchConvergence?.needsConvergence
            ? trOr(t, 'mesh.attention.needsFollowUp', 'Needs follow-up')
            : trOr(t, 'mesh.summary.clean', 'Clean and even with upstream')
    }
    return parts.join(' · ')
}
