...
function Field({ label, value }: { label: string; value: string | number | null }) {
  ...
  return (
    <div className="flex justify-between text-[11px] py-0.5 border-b border-border-subtle/50">
  ...
  return (
    <div className="w-64 rounded-xl border border-border-subtle bg-bg-panel p-4 flex flex-col gap-2 shadow-lg">
      ...
      <div className="flex flex-col gap-0.5 mt-1">
        ...
        <Field label="Ahead" value={node.ahead > 0 ? `+${node.ahead}` : null} />
        <Field label="Behind" value={node.behind > 0 ? `-${node.behind}` : null} />
        ...
        <Field label="Providers" value={node.providers.join(', ') || null} />
      </div>
