        ...
        <div className="border border-border-default/60 rounded bg-surface-secondary p-1.5 space-y-1">
            <div className="flex items-center gap-1.5">
                <select
                    ...
                    className="bg-bg-secondary text-text-primary text-3xs font-mono rounded border border-border-default px-1 py-0.5"
                >
                ...
                {(kind === 'regex' || kind === 'stable' || kind === 'changed' || kind === 'elapsed') && (
                    <button
                        type="button"
                        className="text-3xs text-accent-primary hover:text-accent-primary px-1.5 py-0.5 rounded border border-accent hover:bg-accent-primary/20"
                ...
                {pv && pv !== 'loading' && (
                    <span className={`text-3xs font-mono px-1.5 py-0.5 rounded ${pv.result ? 'text-status-online bg-status-online/20' : 'text-text-secondary bg-bg-glass'}`}>
                        {pv.result ? 'match ✓' : 'no match'}
                    </span>
                )}
                {pv === 'loading' && <span className="text-3xs text-text-muted">testing…</span>}
                {onRemove && (
            ...
            {kind === 'regex' && (
                <div className="flex items-center gap-1">
                    <select
                        ...
                        className="bg-bg-secondary text-accent-primary text-3xs font-mono rounded border border-border-default px-1 py-0.5"
                    >
                    ...
                    <input
                        ...
                        placeholder="regex"
                        spellCheck={false}
                        className="flex-1 bg-bg-secondary text-text-primary text-3xs font-mono rounded border border-border-default px-1.5 py-0.5 outline-none focus:border-accent"
                    />
                </div>
            )}
            {kind === 'elapsed' && (
            ...
            {kind === 'changed' && (
                <div className="flex items-center gap-2">
                    ...
                    <label className="flex items-center gap-1 text-3xs text-text-secondary">
    ...
    return (
        <div className="pl-2 border-l border-border-default space-y-1">
            ...
            <div className="flex items-center gap-1 flex-wrap">
                <button
                    type="button"
                    className="text-3xs text-text-secondary hover:text-text-primary px-1.5 py-0.5 rounded border border-border-default hover:bg-bg-glass-hover"
                ...
                {SNIPPETS.map(s => (
                    <button
                        key={s.label}
                        type="button"
                        className="text-3xs text-accent-primary/80 hover:text-accent-primary px-1.5 py-0.5 rounded border border-accent hover:bg-accent-primary/20"
...
function NumField({ label, value, onChange }: { label: string; value: number; onChange: (v: number) => void }) {
    return (
        <label className="flex items-center gap-1 text-3xs text-text-secondary font-mono">
            {label}
            <input
                type="number"
    ...
    return (
        <div>
            <div className="flex items-center gap-2 mb-1">
                <span className="text-text-secondary text-3xs uppercase tracking-widest font-semibold">Sections</span>
                <span className="text-text-muted text-3xs">— how the screen is split; conditions match against these</span>
                <button
                    type="button"
                    className="ml-auto text-3xs text-accent-primary hover:text-accent-primary px-1.5 py-0.5 rounded border border-accent hover:bg-accent-primary/20"
                    onClick={onTest}
            ...
            {testResult && testResult !== 'loading' && 'error' in testResult && (
                <div className="text-status-error bg-status-error/15 border border-status-error/40 rounded p-1.5 text-3xs mb-1">{testResult.error}</div>
            )}
                        ...
                        <div key={id} className="border border-border-default/60 rounded bg-surface-secondary p-2 space-y-1.5">
                            <div className="flex items-center gap-2">
                                <input
                                    ...
                                    className="w-28 bg-bg-secondary text-accent-primary font-mono text-2xs rounded border border-border-default px-1.5 py-0.5 outline-none focus:border-accent"
                                />
                            ...
                            {mode === 'positional' ? (
                                ...
                                <div className="space-y-1">
                                    <div className="flex items-center gap-1.5">
                                        <span className="text-3xs text-text-muted w-16">anchor</span>
                                        <input
                                            value={def.anchor ?? ''}
                                    ...
                                    <div className="flex items-center gap-1.5">
                                        <span className="text-3xs text-text-muted w-16">ctx next</span>
                                        <input
                                            ...
                                            spellCheck={false}
                                            className="flex-1 bg-bg-secondary text-text-secondary text-3xs font-mono rounded border border-border-default px-1.5 py-0.5 outline-none focus:border-accent"
                                        />
                                    </div>
                                </div>
                            )}
                            ...
                            {previewById[id] && (
                                <div className="border-t border-border-subtle pt-1">
                                    <div className="text-4xs text-text-muted mb-0.5">
                                    ...
                                    <pre className="font-mono text-3xs text-text-secondary whitespace-pre-wrap break-all bg-bg-secondary rounded border border-border-subtle px-2 py-1 max-h-28 overflow-y-auto">
...
export default function SpecFormBuilder({ model, onChange, onPreview, preview, onTestSections, sectionPreview }: Props) {
    ...
    return (
        <div className="space-y-3">
            ...
            <div>
                <div className="flex items-center gap-2 mb-1">
                    <span className="text-text-secondary text-3xs uppercase tracking-widest font-semibold">States</span>
                    <button
                        type="button"
                        className="text-3xs text-text-secondary hover:text-text-primary px-1.5 py-0.5 rounded border border-border-default hover:bg-bg-glass-hover"
                        ...
                        <div key={i} className="border border-border-default/60 rounded bg-surface-secondary p-2 space-y-1">
                            <div className="flex items-end gap-2 flex-wrap">
                                <label className="flex flex-col gap-0.5">
                                    <span className="text-4xs text-text-muted uppercase tracking-wide">id (used by transitions)</span>
                                    <input
                                        value={s.id}
                                        onChange={e => updateState(i, { id: e.target.value })}
                                        placeholder="idle"
                                        className="w-32 bg-bg-secondary text-accent-primary font-mono text-2xs rounded border border-border-default px-1.5 py-0.5 outline-none focus:border-accent"
                                    />
                                </label>
                                <label className="flex flex-col gap-0.5 flex-1">
                                    <span className="text-4xs text-text-muted uppercase tracking-wide">label (shown in dashboard)</span>
                                    <input
                                        value={s.label}
                                        onChange={e => updateState(i, { label: e.target.value })}
                                        placeholder="Ready"
                                        className="w-full bg-bg-secondary text-text-primary text-2xs rounded border border-border-default px-1.5 py-0.5 outline-none focus:border-accent"
                                    />
                                </label>
                            ...
                            <div className="flex items-center gap-3 text-3xs text-text-secondary flex-wrap">
            ...
            <div>
                <div className="flex items-center gap-2 mb-1">
                    <span className="text-text-secondary text-3xs uppercase tracking-widest font-semibold">Transitions</span>
                    <button
                        type="button"
                        className="text-3xs text-text-secondary hover:text-text-primary px-1.5 py-0.5 rounded border border-border-default hover:bg-bg-glass-hover"
                    ...
                    {model.transitions.map((t, i) => {
                        return (
                            <div key={i} className="border border-border-default/60 rounded bg-surface-secondary p-2 space-y-1.5">
                                <div className="flex items-center gap-2 flex-wrap">
                                    <span className="text-3xs text-text-muted">from</span>
                                    ...
                                    <select
                                        ...
                                        className="bg-bg-secondary text-status-online font-mono text-3xs rounded border border-border-default px-1 py-0.5"
                                    >
                                        {stateIds.map(s => <option key={s} value={s}>{s}</option>)}
                                    </select>
                                    <span className="text-3xs text-text-muted ml-1">label</span>
                                    <input
                                        value={t.label ?? ''}
                                        ...
                                        className="flex-1 min-w-[120px] bg-bg-secondary text-text-secondary text-3xs rounded border border-border-default px-1.5 py-0.5 outline-none focus:border-accent"
                                    />
                                ...
                                <div>
                                    <div className="flex items-center gap-2 mb-1">
                                        <span className="text-3xs text-text-muted">when</span>
                                        {!t.when && (
                                            <button
                                                type="button"
                                                className="text-3xs text-text-secondary hover:text-text-primary px-1.5 py-0.5 rounded border border-border-default hover:bg-bg-glass-hover"
                                        ...
                                        {t.when && (
                                            <button type="button" className="text-3xs text-text-muted hover:text-status-error" onClick={() => updateTransition(i, { when: undefined })}>remove guard</button>
                                        )}
                                    </div>
                                    {t.when && (
                                        <ConditionEditor
        ...
        <div className="flex items-center gap-0.5 flex-wrap">
            {states.map(s => (
                <button
                    ...
                    type="button"
                    onClick={() => toggle(s)}
                    className={`font-mono text-3xs px-1 py-0.5 rounded border ${(isWildcard || selected.has(s)) ? 'text-accent-primary bg-accent-primary/20 border-accent' : 'text-text-muted border-border-default hover:border-border-default'}`}
                >{s}</button>
            ))}
        </div>
    )
