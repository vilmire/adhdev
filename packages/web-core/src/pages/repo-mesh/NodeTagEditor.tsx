...
export default function NodeTagEditor({ node, saving, onSave }: Props) {
    ...
    const invalidReason = (raw: string): string | null => {
        const v = raw.trim()
        if (!v) return null
        if (/\s/.test(v)) return t('mesh.tagEditor.noSpaces')
        if (RESERVED_PREFIXES.some(p => v.toLowerCase().startsWith(p))) return t('mesh.tagEditor.reservedPrefix')
        if (custom.includes(v)) return t('mesh.tagEditor.alreadyAdded')
        return null
    }
    ...
    return (
        <div className="mt-2" onClick={e => e.stopPropagation()}>
            <div className="flex flex-wrap items-center gap-1">
                <span className="text-3xs uppercase tracking-wide text-text-muted mr-1">{t('mesh.tagEditor.routingTags')}</span>
                {autoTags.map(at => (
                    <span key={at.tag} className="rounded-full border border-border-subtle bg-bg-secondary px-1.5 py-0.5 font-mono text-3xs text-text-muted" title={t('mesh.tagEditor.autoDerivedTitle')}>
                        {at.tag}
                    </span>
                ))}
                {custom.map(ct => (
                    <span key={ct} className="inline-flex items-center gap-1 rounded-full border border-accent-primary/40 bg-accent-primary/10 px-1.5 py-0.5 font-mono text-3xs text-accent-primary" title={t('mesh.tagEditor.customTagTitle')}>
                        {ct}
                        <button type="button" className="text-accent-primary/70 hover:text-accent-primary bg-transparent border-none cursor-pointer p-0 leading-none"
                            onClick={() => removeTag(ct)} disabled={saving} aria-label={t('mesh.tagEditor.remove', { tag: ct })}>×</button>
                    </span>
                ))}
            </div>
            ...
            <div className="mt-1.5 flex flex-wrap items-center gap-2">
                <input
                    ...
                    placeholder={t('mesh.tagEditor.addPlaceholder')}
                    disabled={saving}
                    className="min-w-[12rem] flex-1 px-2 py-1 rounded-lg bg-bg-secondary border border-border-subtle text-xs text-text-primary"
                />
                <button type="button" className="btn btn-secondary btn-sm" onClick={addDraft} disabled={saving || !draft.trim() || !!draftError}>{t('mesh.tagEditor.add')}</button>
                {dirty && (
                    <button type="button" className="btn btn-primary btn-sm" onClick={() => onSave(custom)} disabled={saving}>
                        {saving ? t('mesh.tagEditor.saving') : t('mesh.tagEditor.saveTags')}
                    </button>
                )}
            </div>
