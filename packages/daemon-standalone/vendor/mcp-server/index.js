...
var import_transcript_read_model_consumers = require("@adhdev/daemon-core/mesh/transcript-read-model-consumers");
...
function narrowReason(value, fallback) {
  const raw = typeof value?.reason === "string" ? value.reason.trim() : "";
  return FALLBACK_REASONS.has(raw) ? raw : fallback;
}
...
async function readTranscriptReplicaForSemanticConsumer(transport, request) {
  if (!(request.roster ?? import_transcript_read_model_consumers.TRANSCRIPT_CONSUMER_ROSTER)[request.consumerId]?.enabled) {
...
function classifyMeshLaunchFailure(error) {
  ...
  const p2pClassification = (0, import_daemon_core7.classifyP2pRelayFailure)(error, { command: "launch_cli" });
...
function buildCoordinatorP2pRelayFailure(error, context) {
  const payload = (0, import_daemon_core7.buildP2pRelayFailurePayload)(error, {
...
function isP2pTransportUnavailableError(error) {
  return (0, import_daemon_core7.isP2pRelayTransportFailure)(error);
}
...
function buildMeshReadChatCacheFallback(ctx, args, node, error) {
  const classification = (0, import_daemon_core7.classifyP2pRelayFailure)(error, { command: "read_chat", targetDaemonId: node.daemonId });
  ...
  const nudgeWedgedReplica = async (task) => {
    ...
    try {
      ...
      if (!payload) {
        ...
        payload = unwrapCommandPayload(read);
      }
...
var import_daemon_core12 = require("@adhdev/daemon-core");
...
async function meshSendTask(ctx, args) {
  ...
  const modeValidation = (0, import_daemon_core8.validateMeshTaskModeRequest)(requestedTaskMode, message, readonly);
...
var import_daemon_core13 = require("@adhdev/daemon-core");
