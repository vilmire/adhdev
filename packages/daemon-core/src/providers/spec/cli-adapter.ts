...
import type { InteractivePrompts, ErrorClassification } from './fsm-types.js';
import { CLAUDE_TUI_REVIEW_PAGE_NOT_FOCUSED_PREFIX } from '@adhdev/mesh-shared';
import {
    stripAnsi,
    classifyDeclaredError,
...
export class SpecCliAdapter implements CliAdapter {
    ...
    private declaredErrorFailure: ClassifiedFailure | null = null;
    ...
    getStatus(_options?: { allowParse?: boolean }): CliAdapterStatus {
        ...
        // A strong live declared-error marker (error_classification — see
        // ERROR-NOT-COMPLETION) outranks generic process liveness. Returning
        // `error` makes CliProviderInstance emit agent:stopped with the typed
        // reason, rather than allowing an idle/exit edge to masquerade as a
        // zero-byte completion (or, for a transport failure mid-turn, a
        // "possible completion (weak evidence)" that quotes the raw error
        // text) or a generic crash eligible for blind recovery.
     ...
     */
    private declaredErrorClassification(): ErrorClassification | undefined {
        if (this.spec.error_classification) return this.spec.error_classification;
        return this.cliType === 'kimi' ? KIMI_DEFAULT_ERROR_CLASSIFICATION : undefined;
    }

    private observeDeclaredErrorOutput(chunk: string, exitCode?: number): boolean {
        ...
        // Kimi's specific operator-facing wording applies only when this
        // adapter is actually running Kimi's DEFAULT classification (the
        // spec hasn't declared its own error_classification yet); any
        // provider with its own declared classification gets the generic
        // per-class message until it likewise declares a message override.
        const usingKimiDefault = !this.spec.error_classification && this.cliType === 'kimi';
        const failure = classifyDeclaredError(this.kimiFailureOutputTail, declared, {
            generating: this.latestState?.status === 'generating',
            messages: usingKimiDefault ? KIMI_ERROR_MESSAGES : undefined,
        });
        if (!failure) return false;
        this.declaredErrorFailure = failure;
