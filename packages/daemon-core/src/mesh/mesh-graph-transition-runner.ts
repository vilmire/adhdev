 ...
 * settle their own attempts); DERIVED FAILURE policy (failed/cancelled rollups,
 * cancel_downstream on task failure) is phase C3. The coordinator GATE contract
 * (claim/release/leases/deadline sweep) landed in phase C2: the runner owns the
 * gate-OPEN transition inside this choke point (design :402-405) plus the
 * settle-time gate guard; the coordinator-facing claim/release/sweep live in
 * mesh-graph-gates.ts and re-enter this file only through its exported
 * settle/advance helpers. `run_if` skip propagation IS implemented here (C1)
 * because it is the other half of condition evaluation — a condition that can
 * only ever block is not a condition. Failure-driven skip is a different axis
 * and stays C3.
 */
