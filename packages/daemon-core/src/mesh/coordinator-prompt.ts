...
function buildPolicySection(policy: RepoMeshPolicy): string {
    ...
    rules.push('- Write tasks are limited to **one active task per node**, so N parallel write tasks need N nodes (clone worktrees). Read-only tasks are exempt and may stack on a node that is already busy');
    rules.push('- Both numbers are ceilings, not targets; most sessions run far under them, so prefer starting independent work over waiting');
