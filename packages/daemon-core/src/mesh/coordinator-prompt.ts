...
function buildPolicySection(policy: RepoMeshPolicy): string {
    ...
    rules.push(`- Maximum **${policy.maxParallelTasks}** tasks running in parallel — this is a ceiling, not a target; most sessions run far under it, so prefer starting independent work over waiting`);
...
- **Bound duplicate/overlapping load, not independent work.** Never launch a second session onto work already in flight for the same issue, and never duplicate a session because \`mesh_read_chat\` shows no final message while tool/terminal activity is ongoing. For independent tasks — especially read-only investigation — dispatch immediately up to headroom under \`maxParallelTasks\` rather than waiting for an in-flight task or a user nudge; the mesh already exempts read-only tasks from the one-active-session-per-node limit, so they need neither a free node nor a worktree. Scale back only when a node actually shows strain, not preemptively. This is about *unrelated* work running side by side; it does not mean splitting one line of work across sessions — successive stages of the same investigation belong in the session that already has the context (see Workflow 3f).
