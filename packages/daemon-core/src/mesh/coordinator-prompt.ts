...
export interface CoordinatorPromptContext {
     ...
     * `.adhdev/coordinator-rules.md`, falling back to the bundled default.
     * Omitted → resolved as bundled inside the builder, so older callers and
     * tests keep working unchanged.
     */
    repoRules?: CoordinatorRulesResolution;
}
...
Repository: \`${mesh.repoIdentity}\`${mesh.defaultBranch ? `\nDefault branch: \`${mesh.defaultBranch}\`` : ''}`);
    ...
    // ── Workflow + Rules — the repo-read operating-rules layer. Resolved at
    //     launch from the mesh base workspace when the caller threaded it in;
    //     bundled default otherwise. This layer is NEVER shed under the soft
    //     cap: rules outrank notes/activity. The compiled safety tail rides
    //     after the rules half so no repo file can remove it. ──
...
function expandPromptPlaceholders(template: string, ctx: CoordinatorPromptContext): string {
    ...
    const replacements: Record<string, string> = {
        ...
        rules: `${rulesLayer.rules}\n\n${buildSafetyTailSection(coordinatorCliType)}`,
...
function buildSafetyTailSection(coordinatorCliType?: string): string {
    ...
    // that can turn it off, and it deliberately lives OUTSIDE the repo-read rules
    // layer so no repo's coordinator-rules.md can remove it.
