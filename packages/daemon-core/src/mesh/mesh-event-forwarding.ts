...
import {
    bootstrapQueueTaskCountsAsHandled,
    buildRelayMetadataEvent,
    flushPendingForMeshIdleCoordinators,
    isMidGenerationSplitEligible,
    nonRetryableProviderFailureReason,
    resolveCoordinatorDrainDaemonIds,
    shouldRequeueHollowCompletion,
} from './mesh-event-delivery.js';
// Re-exported for backward compatibility: these now live in mesh-event-delivery.ts
// (pure move, file-size gate decomposition — mission B1) but external importers still
// reach them via mesh-event-forwarding.js and the mesh-events-coordinator.ts barrel
// (test/mesh/mesh-relay-metadata-taskid.test.ts, test/mesh/mesh-notif-event-driven-drain.test.ts,
// test/mesh/coordinator-delivery-modes.test.ts, test/commands/refine-event-return-path.test.ts).
// `nonRetryableProviderFailureReason` and `resolveCoordinatorDrainDaemonIds` are NOT
// re-exported — they were file-private before the move and only this file consumes them.
export {
    bootstrapQueueTaskCountsAsHandled,
    buildRelayMetadataEvent,
    flushPendingForMeshIdleCoordinators,
