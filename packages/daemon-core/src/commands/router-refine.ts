...
// REFINE-JOB-PLUMBING: job handle / coordinator event / ledger helpers moved to
// router-refine-jobs.ts (file-size gate, pure move). Re-exported so existing
// importers — and the tests that import them from here — keep their path.
export {
    buildRefineJobKey,
...
// REFINE-RESUME-SCAN: the boot-time stale-dispatch reconciliation pass moved to
// router-refine-resume.ts (file-size gate). Re-exported so existing importers
// (boot/daemon-lifecycle.ts) keep their import path unchanged.
export { resumePendingRefineJobsOnStartup } from './router-refine-resume.js';
...
// REFINE-PREFLIGHT-STAGES: the patch_equivalence / submodule_reachability /
// effective_diff stages moved to router-refine-preflight-stages.ts (file-size
// gate, pure move). Re-exported so existing importers keep their path.
export {
...
// Batch Refinery orchestration (batch plan/ordering, convergence core, async batch
// job lifecycle + event/ledger plumbing) — pure move to keep this file under its
// frozen file-size baseline (router-refine-resume.ts precedent). Re-exported here
// so existing import sites (router.ts, tests) are unaffected. The convergence
// classifier stays below because the single-node auto-retry shares it.
export {
    batchRefineMeshNodes,
    runMeshRefineBatchConvergence,
    buildRefineBatchJobKey,
    buildRefineBatchJobHandle,
    queueRefineBatchJobEvent,
    appendRefineBatchJobLedger,
    finishMeshRefineBatchJob,
