...
// ── Provider native-history read path ─────────────────────────────────────────
// Moved to ./provider-native-history.ts (FILE-SIZE-HEADROOM). Re-exported here
// so every existing `from './chat-history.js'` import keeps resolving.
export {
    materializeProviderNativeHistory,
    isNativeSourceCanonicalHistory,
    readProviderChatHistory,
    listProviderHistorySessions,
} from './provider-native-history.js';
export type { ProviderNativeHistoryScripts } from './provider-native-history.js';
