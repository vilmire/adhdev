...
describe('default tarball extraction (Node-native, no system tar)', () => {
  ...
  it.runIf(hasSystemTar)('produces the same file set and tree digest as the system tar extraction', async () => {
    ...
    execFileSync('tar', ['-xzf', forSystemTar(tarPath), '-C', forSystemTar(sysOut), ...forceLocalArgs()], { env: SYSTEM_TAR_ENV });
    ...
    execFileSync('tar', ['-czf', forSystemTar(sysMadeTar), '-C', forSystemTar(tarSrcParent), '.', ...forceLocalArgs()], { env: SYSTEM_TAR_ENV });
