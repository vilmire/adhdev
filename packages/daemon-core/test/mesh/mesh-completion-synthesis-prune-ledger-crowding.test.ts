...
// SAME kind set the fix now reads (readLocalRecordsByKind), mirroring what
...
describe('mesh-completion-synthesis auto-prune — LEDGER-KIND-TAIL-BLINDSPOT', () => {
    ...
    // `readLocalRecords(meshId, { tail: 500 })`; a bare tail window can be crowded out by
    ...
    it('★a buried terminal keeps a remote dispatch in terminalDirectWork, not the orphan-prune bucket', () => {
        ...
        const ledgerEntries = readLocalRecordsByKind(meshId, [...AUTO_PRUNE_LEDGER_KINDS]);
