...
> **Release history source of truth: git log + npm dist-tags.**
> This file is maintained by hand and is no longer auto-scaffolded by
> `scripts/version-bump.sh`. Entries for versions after 0.9.62 were never
> backfilled here — see `git log` (and `npm dist-tags` on the
> published packages) for the authoritative release history of 0.9.63 and
> later, including the 1.0.x line.
...
### Added
- **web-core**: Support detaching Dockview dashboard tabs into floating panels by dragging a tab, with regression coverage for the drag-to-float wiring.
...
### Changed
- Refreshed self-hosted provider configuration docs and completed the 0.9.6-0.9.35 changelog backfill.
